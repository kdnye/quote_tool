...
def _first_numeric_in_column(series: pd.Series) -> float:
    """Return the first numeric-looking value in the column ($, commas, % ok)."""
...
def _accessorial_prices(selected_names):
    """
    Return ([(name, price), ...], subtotal) by reading the Accessorials sheet headers.
    Guarantee is excluded here (handled as a separate line item).
    """
    try:
        ...
        # If workbook can't be read, fall back to zero pricing
        return [(name, 0.0) for name in selected_names if "guarantee" not in str(name).lower()], 0.0
...
def email_form_ui():
    ...
    guarantee_selected = bool(
        ...
        or any("guarantee" in s.lower() for s in selected_accessorials)
    )
    ...
    # ---------- EMAIL BODY FORMATTING (restored & aligned) ----------
    def _fmt_money(x: float) -> str:
        return f"${x:,.2f}"

    NAME_COL = 28  # adjust spacing if you like
    AMT_COL = 12
    SEP = "-" * 12
    ...
    # Build accessorial rows & subtotals from workbook headers
    acc_rows, acc_subtotal = _accessorial_prices(selected_accessorials)

    # Derive a presentational guarantee amount (for Air only).
    ...
    # Guarantee: single aligned line, then subtotal with separator
    ...
    lines.append("")  # blank line
    # Quote Total shows the email total and notes the fee
    ...
    # Launch mail client button (kept from your current version)
    st.markdown(
        f"""
